/*
 * ///my copied from CommandLineLoaderTest, but tests CommandLineLoaderII
 */
...
public class CommandLineLoaderIITest {
  ...
  public void testCommandLineLoader() {
    // Test with no args.
    CommandLineLoaderII loader = new CommandLineLoaderII(emptyArgv);
    assertEquals(0, loader.getNodeArguments().size());

    // Test with no remappings.
    emptyArgv.add("one two --three");
    loader = new CommandLineLoaderII(emptyArgv);
    ...
    for (String test : tests) {
      loader = new CommandLineLoaderII(Lists.newArrayList(("Foo " + test).split("\\s+")));
  ...
  public void testcreateConfigurationEnvironment() {
    ...
    CommandLineLoaderII loader = null;
    env = Maps.newHashMap();
    env.put(EnvironmentVariables.ROS_ROOT, defaultRosRoot.getAbsolutePath());
    loader = new CommandLineLoaderII(emptyArgv, env);
  ...
  public void testCreateConfigurationCommandLine() throws URISyntaxException {
    ...
    NodeConfiguration nodeConfiguration = new CommandLineLoaderII(emptyArgv, env).build();
    assertEquals(null, nodeConfiguration.getNodeName());
    List<String> args = Lists.newArrayList("Foo", "__name:=newname");
    nodeConfiguration = new CommandLineLoaderII(args, env).build();
  ...
  public void testcreateConfigurationResolver() {
    ...
    // Test with no args.
    CommandLineLoaderII loader = new CommandLineLoaderII(emptyArgv, env);
    NodeConfiguration nodeConfiguration = loader.build();
    ...
    loader = new CommandLineLoaderII(args, env);
    nodeConfiguration = loader.build();
